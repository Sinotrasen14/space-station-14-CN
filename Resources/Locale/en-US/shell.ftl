...
shell-can-only-run-from-pre-round-lobby = 你只能在游戏处于等待大厅时运行这个命令
shell-can-only-run-while-round-is-active = 你只能在游戏回合内运行此命令
...
shell-must-have-body = 你必须有一个身体才能运行此命令
...
shell-missing-required-permission = 你需要提供 {$perm} 给从命令！
...
shell-entity-target-lacks-component = 目标实体缺少 {INDEFINITE($componentName)} {$componentName} 组件
